...
class Graph{
  ...
  public :
    ...
    string reconstructed(string start, string end, unordered_map<string, string> prev)
    {
      deque<string> path;
    ...
    string bfs(string start, string end)
    {
      ...
      unordered_map<string, bool> visited (V); // initlize vector of bool to false wisth size of the number of vectors
      unordered_map<string, string> prev(V); // initilaize string vector of prev
      for(auto a : _v){
        visited[a] = false;
        prev[a] = "";
      }
      ...
      while(!Q.empty()){
        ...
        for(auto next = neighbors.begin(); next != neighbors.end(); next++){
          ...
          if(!visited[next->first]){ // check is visted
